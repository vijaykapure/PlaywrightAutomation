...
Call log:
  - waiting for locator('//a[contains(text(),\'Add to cart\')]')
        ...
        - generic [ref=e86]:
          ...
          - generic [ref=e88]:
            - heading "Sony xperia z5" [level=4] [ref=e89]:
              - link "Sony xperia z5" [active] [ref=e90] [cursor=pointer]:
                - /url: prod.html?idp_=6
...
```ts
  ...
  2  |     constructor(page) {
  3  |         this.page = page;
  4  |         this.addToCartBtn = page.locator("//a[contains(text(),'Add to cart')]");
  5  |     }
  6  | 
  7  |     async selectProduct(productname) {
  8  |         const product = this.page.locator("//a[contains(text(),'" + productname + "')]");
  9  |         await product.click();
  10 |     }
  11 | 
  12 |     getProductLocator(productname) {
  13 |         return this.page.locator("//a[contains(text(),'" + productname + "')]");
  14 |     }
  15 | 
  16 |     async clickAddToCart() {
> 17 |         await this.addToCartBtn.click();
     |                                 ^ Error: locator.click: Test timeout of 40000ms exceeded.
  18 |         this.page.once('dialog', async dialog => {
  19 |             await dialog.accept();
  20 |         });
  21 |     }
  22 | }
  23 | 
  24 | module.exports = { DashboardPage };
```